...
| `sources` | n/a | **8** | Source/remap pairs, generated at image build. Not a decode-parallelism knob — it sets how many serial `send_outputs` paths run in parallel (see Topology). Its cost is memory: remap in-flight residency scales with it, and it is coupled to `chunk_size_events`. |
| `chunk_size_events` | 1000 | **1000** | `VECTOR_CHUNK_SIZE_EVENTS`. Scales `ready_array_capacity` (×4) and the source-sender buffer together, so it sets the runner's in-flight budget: 32 in-flight chunks × 4×this × the 100:1 fan-out, per remap. Left at the shipped value until measured. Sweepable. |
...
| `transform` | n/a | **`fused`** | Selects the VRL program. `fused` is [`transform.vrl`](transform.vrl); `twopass` is the same transform in the spelling that preceded it, kept so a configuration search can measure the rewrite against its own A/A control rather than across runs. Both produce byte-identical rows. |
...
in a swapfile. `FORMAT` and `SOURCES` together select the config file and
`TRANSFORM` the VRL program; the rest of the knobs arrive as `VECTOR_*` and
`SINK_*`. **That recipe runs the image's defaults**, which are kept equal to the
...
To be answered from a profile of the measured run before publication. The
candidate is per-row cost in Vector's event model rather than any one stage:
the arm spends its whole 32-CPU envelope, ClickHouse sits near idle behind it,
and of the ~15 µs of CPU per output row only ~1.0–1.5 µs is Kafka and Avro
decode. The rest is spread across VRL execution, the per-row metadata clone and
size walk on each remap's output path, and encoding a twelve-key `ObjectMap`
per row. The evidence to cite is the profile plus `cpu_us_per_row` against
`ch_cpu_us_per_row`.
